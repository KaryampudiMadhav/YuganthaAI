...
router.post("/", async (req, res) => {
    try {
        ...
        if (leadType === "Brochure") {
            const existingLead = await Lead.findOne({ phone, courseId, type: "Brochure" });
            if (existingLead) {
                // If already downloaded, just return success (or separate message if needed)
                // We return the brochure link or just success. 
                // Since frontend handles download, we just say "OK"
                return res.status(200).json({ message: "Brochure already requested.", lead: existingLead });
            }
        }
